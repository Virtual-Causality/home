...
public class ColorCopy {
	private static final String CSH_NAME = "/home/vcausality/launch/csh14.spv";
	...
	public ColorCopy(List<Long> dstImageViews, int swapchainSize) {
	...
	public ColorCopy createPipeline() {
		try(var stack = stackPush()) {
	...
	public void drawColor(VkCommandBuffer c, int imageIdx, ShaderUBOInfo si) {
		try(var stack = stackPush()) {
			...
			vkCmdBindDescriptorSets(
					...
					null);
			vkCmdDispatch(c,
					ColorCopy.calcDispatchSize(si.cpyRegion().x()),
					ColorCopy.calcDispatchSize(si.cpyRegion().y()),
					1);
		}
	}
