...
public class ASCII8x20Copy {
	private static final String CSH_NAME = "/home/vcausality/launch/csh08.spv";
	...
	public ASCII8x20Copy(long srcImageView, List<Long> dstImageViews, int swapchainSize) {
	...
	public ASCII8x20Copy createPipeline() {
		try(var stack = stackPush()) {
	...
	public void drawASCII8x20(VkCommandBuffer c, int imageIdx, int x, int y, String mes) {
		try(var stack = stackPush()){
			var cdes = getCdes(stack, x, y, mes);
			vkCmdPushConstants(c, this.getPipelineLayout(), VK_SHADER_STAGE_COMPUTE_BIT, 0, cdes);
			...
			vkCmdBindDescriptorSets(
					...
					null);
			vkCmdDispatch(c, 1, 1, mes.length() > 40 ? 40 : mes.length());
		}
	}
	...
	public static ByteBuffer getCdes(MemoryStack stack, int dstx, int dsty, String str) {
		int l = str.length() > 40 ? 40 : str.length();
		var cdes = stack.calloc(48);
		cdes.putInt(0, dstx);
		cdes.putInt(4, dsty);
		for(int i = 0; i < l; i++) {
			cdes.put(i + 8, (byte)((str.codePointAt(i) - 32) & 255));
		}
		return cdes.rewind();
	}
}
