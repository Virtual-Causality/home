...
public class CellCopy {
	private static final String CSH_NAME = "/home/vcausality/launch/csh10.spv";
	...
	public CellCopy createPipeline() {
		try(var stack = stackPush()) {
	...
	public static IntBuffer createFixedPositions(MemoryStack stack) {
		var d = stack.mallocInt(520 * 2);
		loop(d, 28, 14, 205, 156);
		loop(d,  4,  4, 101,  52);
		loop(d,  4,  4, 205,  52);
		loop(d,  4,  4, 309,  52);
		loop(d,  4,  4, 413,  52);
	...
	private static void loop(IntBuffer ib, int h, int w, int y, int x) {
		int yy = y;
		int xx;
		for(int i = 0; i < h; i++) {
			xx = x;
	...
	public void drawCells(VkCommandBuffer c, int imageIdx) {
		try(var stack = stackPush()){
